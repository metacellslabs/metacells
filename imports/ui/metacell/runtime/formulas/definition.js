export function defineFormula(definition) {
  ...
  const name = String(source.name || "").trim().toUpperCase();
  if (!name) {
  ...
  return {
    ...
    aliases: Array.isArray(source.aliases)
      ? source.aliases.map((alias) => String(alias || "").trim().toUpperCase()).filter(Boolean)
      : [],
    summary: String(source.summary || "").trim(),
    signature: String(source.signature || `${name}(...)`).trim(),
    examples: Array.isArray(source.examples) ? source.examples.map((item) => String(item || "").trim()).filter(Boolean) : [],
...
export function validateFormulaDefinition(definition, origin) {
  ...
  if (!source) {
    throw new Error(`Formula module ${location} must export a default formula definition object`);
  }
  ...
  if (typeof source.execute !== "function") {
    throw new Error(`Formula module ${location} must provide an execute(args) function`);
  }
