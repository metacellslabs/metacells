...
export default defineFormula({
  ...
  summary: "Looks for a value in the first column of a table and returns a value from another column.",
  ...
  execute: ({ args, helpers }) => {
    ...
    for (let i = 0; i < table.length; i += 1) {
      ...
      if (String(firstCell) === String(lookupValue)) {
        return typeof row[columnIndex - 1] === "undefined" ? "" : row[columnIndex - 1];
      }
      if (!exactMatch && helpers.isNumberLike(firstCell) && helpers.isNumberLike(lookupValue)) {
    ...
    if (approximateRow) {
      return typeof approximateRow[columnIndex - 1] === "undefined" ? "" : approximateRow[columnIndex - 1];
    }
    return "";
  },
});
