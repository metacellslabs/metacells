...
preprocessFormula(formula, sourceCellId) {
        ...
        var withAtSheetRefs = withRawAtSheetRefs.replace(/@(?:'([^']+)'|([A-Za-z][A-Za-z0-9 _-]*))!([A-Za-z]+[0-9]+)/g, (_, quoted, plain, cellId) => {
            var sheetName = quoted || plain || "";
            return "mentionSheetRef(\"" + this.escapeForDoubleQuotedString(sheetName) + "\",\"" + cellId.toUpperCase() + "\")";
        });
        var withSheetRegionRefs = withAtSheetRefs.replace(/(?:'([^']+)'|([A-Za-z][A-Za-z0-9 _-]*))!([A-Za-z]+[0-9]+):([A-Za-z]+[0-9]+)/g, (_, quoted, plain, startCellId, endCellId) => {
            var sheetName = quoted || plain || "";
            return "sheetRegionRef(\"" + this.escapeForDoubleQuotedString(sheetName) + "\",\"" + startCellId.toUpperCase() + "\",\"" + endCellId.toUpperCase() + "\")";
        });
        var withRegionRefs = withSheetRegionRefs.replace(/([A-Za-z]+[0-9]+):([A-Za-z]+[0-9]+)/g, (_, startCellId, endCellId) => {
            return "regionRef(\"" + startCellId.toUpperCase() + "\",\"" + endCellId.toUpperCase() + "\")";
        });
        var withSheetRefs = withRegionRefs.replace(/(?:'([^']+)'|([A-Za-z][A-Za-z0-9 _-]*))!([A-Za-z]+[0-9]+)/g, (_, quoted, plain, cellId) => {
            var sheetName = quoted || plain || "";
            return "sheetRef(\"" + this.escapeForDoubleQuotedString(sheetName) + "\",\"" + cellId.toUpperCase() + "\")";
        });
        var withStringMentions = this.interpolateMentionsInStringLiterals(withSheetRefs);
        return withStringMentions.replace(/(_)?@([A-Za-z_][A-Za-z0-9_]*)/g, (_, rawPrefix, token) => {
            ...
            return rawMode
                ? "mentionRawNamedRef(\"" + this.escapeForDoubleQuotedString(token) + "\")"
                : "mentionNamedRef(\"" + this.escapeForDoubleQuotedString(token) + "\")";
        });
    },
...
interpolateMentionsInStringContent(content) {
        ...
        var pattern = /(_)?@(?:'([^']+)'|([A-Za-z][A-Za-z0-9 _-]*))!([A-Za-z]+[0-9]+)|(_)?@([A-Za-z_][A-Za-z0-9_]*)/g;
        var parts = [];
        var cursor = 0;
        var m;
        ...
        while ((m = pattern.exec(text))) {
            ...
            if (sheetCell) {
                ...
                parts.push({
                    kind: "expr",
                    value: (sheetRawPrefix === "_"
                        ? "mentionRawSheetRef(\"" + this.escapeForDoubleQuotedString(sheetName) + "\",\"" + sheetCell.toUpperCase() + "\")"
                        : "mentionSheetRef(\"" + this.escapeForDoubleQuotedString(sheetName) + "\",\"" + sheetCell.toUpperCase() + "\")")
                });
            } else if (plainToken) {
                ...
                if (this.isExistingCellId(plainToken)) {
                    ...
                    parts.push({
                        kind: "expr",
                        value: rawMode
                            ? "mentionRawNamedRef(\"" + this.escapeForDoubleQuotedString(plainToken) + "\")"
                            : "mentionNamedRef(\"" + this.escapeForDoubleQuotedString(plainToken) + "\")"
                    });
                }
            }
...
preprocessUpdateTargetArg(rawArg, sourceCellId) {
        ...
        var quotedCell = /^["']((?:'[^']+'|[A-Za-z][A-Za-z0-9 _-]*)![A-Za-z]+[0-9]+)["']$/.exec(value);
        if (quotedCell) return "\"" + this.escapeForDoubleQuotedString(quotedCell[1]) + "\"";

        var quotedLocal = /^["']([A-Za-z]+[0-9]+)["']$/.exec(value);
        if (quotedLocal) return "\"" + quotedLocal[1].toUpperCase() + "\"";

        var sheetCell = /^(?:'([^']+)'|([A-Za-z][A-Za-z0-9 _-]*))!([A-Za-z]+[0-9]+)$/.exec(plain);
        if (sheetCell) {
            var sheetName = sheetCell[1] || sheetCell[2] || "";
            var cellId = sheetCell[3].toUpperCase();
            if (sheetCell[1]) return "\"'" + this.escapeForDoubleQuotedString(sheetName) + "'!" + cellId + "\"";
            return "\"" + this.escapeForDoubleQuotedString(sheetName) + "!" + cellId + "\"";
        }
...
findClosingParen(text, openParenIndex) {
        ...
        if (openParenIndex < 0 || openParenIndex >= content.length || content.charAt(openParenIndex) !== "(") return -1;
