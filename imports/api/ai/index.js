      ...
      content: await Promise.all(content.map(async (part) => {
        ...
        const imageUrl =
          part.image_url && part.image_url.url
            ...
            : '';
        if (!imageUrl || /^[a-z]+:\/\//i.test(imageUrl) || imageUrl.startsWith('data:')) {
          return part;
        }
        const artifactMatch = imageUrl.match(/^\/artifacts\/([^/?#]+)$/);
        if (artifactMatch) {
          const artifactId = decodeURIComponent(String(artifactMatch[1] || ''));
          ...
          if (binary && binary.buffer) {
            return {
              ...
              image_url: {
                ...(part.image_url || {}),
                url: `data:${String(binary.mimeType || 'application/octet-stream')};base64,${binary.buffer.toString('base64')}`,
              },
            };
          }
        }
        const absoluteUrl = Meteor.absoluteUrl(imageUrl.replace(/^\/+/, ''));
        return {
          ...part,
          image_url: {
            ...(part.image_url || {}),
...
function messagesContainImageParts(messages) {
...
export function extractOpenAIResponsesText(data) {
  ...
  return output
    .flatMap((item) => {
      const content =
        item && item.content && Array.isArray(item.content) ? item.content : [];
      return content;
    })
    .map((part) => {
      if (!part || typeof part !== 'object') return '';
...
async function runAIChatJob(job) {
  ...
  if (
    ...
    if (!rebuilt || !Array.isArray(rebuilt.messages) || !rebuilt.messages.length) {
      log('method.ai.requestChat.cancelled', {
        sourceCellId:
          task.queueMeta && task.queueMeta.sourceCellId
            ? String(task.queueMeta.sourceCellId || '').toUpperCase()
            : '',
        formulaKind: String((task.queueMeta && task.queueMeta.formulaKind) || ''),
      });
      return '';
    }
    if (rebuilt && Array.isArray(rebuilt.messages) && rebuilt.messages.length) {
  ...
  const provider = await getActiveAIProvider();
  const sourceMessages = task.payload.messages || [];
  const supportsImages = providerSupportsImageInput(provider, model);
  if (!supportsImages && messagesContainImageParts(sourceMessages)) {
    throw new Error(
      `The selected model "${model}" does not support image input. Choose a vision-capable model/provider.`,
    );
  }
  const effectiveMessages = supportsImages
    ? sourceMessages
    : stripUnsupportedImageParts(sourceMessages);
  const requestMessages = await materializeAiMessageUrls(effectiveMessages);
  ...
  const useOpenAIResponsesApi =
    provider.type === 'openai' && messagesContainImageParts(requestMessages);
  const requestUrl = `${requestBaseUrl.replace(/\/+$/, '')}/${useOpenAIResponsesApi ? 'responses' : 'chat/completions'}`;
  ...
  const requestPayload = useOpenAIResponsesApi
    ? {
        model,
        input: buildOpenAIResponsesInput(requestMessages),
      }
    : {
        model,
  ...
  if (useOpenAIResponsesApi) {
    finalContent = String(extractOpenAIResponsesText(data) || '');
  } else {
    const message =
