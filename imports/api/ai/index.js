...
const queueMetaMatch = Match.Maybe(
  Match.Where((value) => value == null || (!!value && typeof value === "object" && !Array.isArray(value))),
);
...
async function fetchModelFromServer() {
  ...
  if (cachedModel && cachedModel.providerKey === providerKey) {
    log("model.cache_hit", { model: cachedModel.model, provider: provider.type });
  ...
  const lmStudioBaseUrl = provider.type === "lm_studio" ? provider.baseUrl : await getLMStudioBaseUrl();
  log("model.fetch.start", { provider: provider.type, baseUrl: lmStudioBaseUrl });
  ...
  try {
    ...
    log("model.fetch.success", { model: cachedModel.model, provider: provider.type });
    return cachedModel.model;
  } catch (error) {
    cachedModel = { providerKey, model: "local-model" };
    log("model.fetch.fallback", { error: error.message, model: cachedModel.model, provider: provider.type });
    return cachedModel.model;
  }
}
...
function createTaskKey(type, payload, queueMeta) {
  if (queueMeta && queueMeta.queueIdentity) {
    return `${type}:${queueMeta.queueIdentity}`;
  }
...
function scheduleQueueDrain() {
  Meteor.defer(() => {
    drainQueue().catch((error) => {
      log("queue.drain.error", { message: error && error.message ? error.message : String(error) });
    });
  });
}
...
function normalizeTaskDependencies(dependencies) {
  return Array.isArray(dependencies)
    ? dependencies.filter((dependency) => dependency && typeof dependency === "object")
    : [];
}

function createSourceLockKey(meta) {
  if (!meta || !meta.sheetDocumentId || !meta.activeSheetId || !meta.sourceCellId) return "";
  return [
...
function createQueueIdentity(queueMeta) {
  if (!queueMeta || !queueMeta.sheetDocumentId || !queueMeta.sourceCellId || !queueMeta.formulaKind) {
    return "";
  }
  return [
...
function shouldRefreshTaskForChanges(task, changes) {
  const dependencies = normalizeTaskDependencies(task.queueMeta && task.queueMeta.dependencies);
    ...
    return dependencies.some((dependency) => {
      ...
      if (dependency.kind === "cell") {
        return (
          ...
          String(change.cellId || "").toUpperCase() === String(dependency.cellId || "").toUpperCase()
        );
      }

      if (dependency.kind === "region" && change.kind === "cell") {
        if (String(change.sheetId || "") !== String(dependency.sheetId || "")) return false;
...
async function buildQueuedPayload(queueMeta) {
  if (!queueMeta || !queueMeta.sheetDocumentId || typeof loadSheetDocumentStorageHook !== "function") {
    return null;
  }

  const workbookData = await loadSheetDocumentStorageHook(queueMeta.sheetDocumentId);
  ...
  return aiService.withRequestsSuppressed(() => {
    ...
    if (queueMeta.formulaKind === "list") {
      const count = Math.max(1, Math.min(50, parseInt(queueMeta.count, 10) || 5));
      const messages = [];
...
async function refreshTaskFromSheetState(task, reason) {
  if (!task || task.state !== "queued" || !task.queueMeta || typeof loadSheetDocumentStorageHook !== "function") {
    return;
  }
...
function enqueueTask(type, payload, queueMeta, runner) {
  ...
  if (existing) {
    ...
    log("queue.dedupe", { taskKey, state: existing.state, attempts: existing.attempts });
  ...
  log("queue.enqueued", {
    ...
    dependencies: normalizeTaskDependencies(normalizedMeta && normalizedMeta.dependencies).length,
  });
  scheduleQueueDrain();
  return promise;
}
...
export async function notifyQueuedSheetDependenciesChanged(sheetDocumentId, changes) {
  ...
  for (let i = 0; i < aiQueuedTasks.length; i += 1) {
    ...
    if (String(task.queueMeta.sheetDocumentId || "") !== normalizedSheetId) continue;
...
if (Meteor.isServer) {
  Meteor.methods({
    "ai.setSourceEditLock"(sheetDocumentId, activeSheetId, sourceCellId, locked, ownerId, sequence) {
    ...
    async "ai.fetchUrlMarkdown"(url) {
      ...
      const finalMarkdown =
        markdown.length > URL_MARKDOWN_MAX_CHARS ? markdown.slice(0, URL_MARKDOWN_MAX_CHARS) : markdown;
