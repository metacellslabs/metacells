  ...
  return {
    ...
    availableModels: Array.isArray(source.availableModels)
      ? source.availableModels
          ...
          .filter(Boolean)
      : [],
    credentialLinks: Array.isArray(source.credentialLinks)
      ? source.credentialLinks
          .filter((link) => link && typeof link === 'object' && link.label && link.url)
          .map((link) => ({
            label: String(link.label || '').trim(),
            url: String(link.url || '').trim(),
          }))
      : [],
    credentialSteps: Array.isArray(source.credentialSteps)
      ? source.credentialSteps
...
export function validateAIProviderDefinition(definition, origin) {
  ...
  if (!source) {
    throw new Error(
      `AI provider module ${location} must export a default provider definition object`,
    );
  }
