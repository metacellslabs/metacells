...
async function sendTelegramAttachment(token, chatId, attachment, caption) {
  ...
  const fileName = String(
    source.name ||
      binary.fileName ||
      (type.indexOf('image/') === 0 ? 'image' : 'attachment'),
  ).trim();
  const mimeType = String(
    source.type || binary.mimeType || 'application/octet-stream',
  ).trim();
...
const TELEGRAM_HANDLER = defineChannelHandler({
  ...
  name: 'Telegram',
  summary:
    'Telegram Bot API channel for outbound messages, inbound polling, and attachment downloads.',
