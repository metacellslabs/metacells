...
export class StorageService {
  ...
  inferTabsFromStorage() {
    var sheetIds = [];
    var reportIds = [];
    var workbookSheetIds = this.storage.listSheetIds
      ? this.storage.listSheetIds()
      : [];

    for (var i = 0; i < workbookSheetIds.length; i++) {
      var sheetId = workbookSheetIds[i];
      var reportContent = this.storage.getReportContent
        ? this.storage.getReportContent(sheetId)
        : '';
    ...
    for (var n = 0; n < namedSheetIds.length; n++) {
      if (sheetIds.indexOf(namedSheetIds[n]) === -1)
        sheetIds.push(namedSheetIds[n]);
    }
  ...
  getCellFormat(sheetId, cellId) {
    if (typeof this.storage.getCellFormat === 'function') {
      return this.storage.getCellFormat(sheetId, cellId) || 'text';
    }
    return 'text';
  }

  getCellPresentation(sheetId, cellId) {
    if (typeof this.storage.getCellPresentation === 'function') {
      return (
        this.storage.getCellPresentation(sheetId, cellId) || {
          format: 'text',
          align: 'left',
          wrapText: false,
          bold: false,
  ...
  getCellProcessedChannelEventIds(sheetId, cellId) {
    if (typeof this.storage.getCellProcessedChannelEventIds === 'function') {
      return (
        this.storage.getCellProcessedChannelEventIds(sheetId, cellId) || {}
      );
    }
    return {};
  }
  ...
  getCellVersionInfo(sheetId, cellId) {
    if (typeof this.storage.getCellVersionInfo === 'function') {
      return (
        this.storage.getCellVersionInfo(sheetId, cellId) || {
          sourceVersion: 0,
          computedVersion: 0,
          dependencyVersion: 0,
          dependencySignature: '',
  ...
  setComputedCellValue(sheetId, cellId, value, state, errorMessage, meta) {
    if (typeof this.storage.setComputedCellValue === 'function') {
      this.storage.setComputedCellValue(
        sheetId,
        cellId,
        value,
        state,
  ...
  rewriteFormulaReferencesOnRename(oldName, newName) {
    ...
    for (var s = 0; s < sheetIds.length; s++) {
      var ids = this.storage.listCellIds
        ? this.storage.listCellIds(sheetIds[s])
        : [];
  ...
  rewriteFormulaSheetRefs(rawValue, oldName, newName) {
    ...
    return rawValue.replace(
      ...
      function (match, quoted, plain, cellId) {
  ...
  getCacheValue(key) {
    return this.storage.getCacheValue
      ? this.storage.getCacheValue(key)
      : undefined;
  }
  ...
  setCellName(sheetId, cellId, name, rangeRef) {
    ...
    if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(cleaned)) {
      return {
        ok: false,
        error:
          'Name must start with a letter or _, and use only letters, numbers, _.',
      };
    }
  ...
  namedRefMatches(left, right) {
    ...
    var leftStart = left.startCellId
      ? String(left.startCellId).toUpperCase()
      : '';
    var rightStart = right.startCellId
      ? String(right.startCellId).toUpperCase()
      : '';
    ...
    return (
      leftCell === rightCell && leftStart === rightStart && leftEnd === rightEnd
    );
  }
